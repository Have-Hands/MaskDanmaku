...
// 重置画布高度，避免弹幕遮挡视频播放控件
barrage.canvas.height = container.clientHeight;
...
// 实时绘制视频到画布
barrage.afterRender = () => {
    var progress = video.currentTime * 1000;
  var fps = 30;
  var frames = Math.max(parseInt(progress*fps/1000),0);
  // console.log(frames)
  var extra_frame = 1;
  var path = "./mask1/"
  // console.log(frame);
  if(frames% extra_frame==0){
      var path_img = path+frames+".png";
      const image = new Image;
      image.src =path_img;
      image.onload = () => {   
          vContext.drawImage(image,0, 0, vCanvas.width, vCanvas.height);
    }
  }
};
